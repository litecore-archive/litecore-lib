 ...
 *
 * @param {Address|String} litecoinAddress - A bitcoin address
 * @param {String} signatureString - A base64 encoded compact signature
 * @returns {Boolean}
 */
Message.prototype.verify = function verify(litecoinAddress, signatureString) {
  $.checkArgument(litecoinAddress);
  $.checkArgument(signatureString && _.isString(signatureString));

  if (_.isString(litecoinAddress)) {
    litecoinAddress = Address.fromString(litecoinAddress);
  }
  ...
  var signatureAddress = Address.fromPublicKey(publicKey, litecoinAddress.network);

  // check that the recovered address and specified address match
  if (litecoinAddress.toString() !== signatureAddress.toString()) {
