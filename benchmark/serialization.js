...
async.series([
  function(next) {
    ...
    for (var i = 0; i < 100; i++) {
      ...
      // uint64le
      var br = new bibecore.encoding.BufferWriter();
      var num = Math.round(Math.random() * 10000000000000);
      br.writeUInt64LEBN(new bibecore.crypto.BN(num));
      buffers.push(br.toBuffer());

      // hashes
      var data = bibecore.crypto.Hash.sha256sha256(new Buffer(32));
    ...
    function readUInt64LEBN() {
      ...
      var buf = buffers[c];
      var br = new bibecore.encoding.BufferReader(buf);
  ...
  function(next) {
    ...
    function bibecoreTest() {
      block1 = bibecore.Block.fromString(blockData);
    }
    ...
    var suite = new benchmark.Suite();
    suite.add('bibecore', bibecoreTest, {maxTime: maxTime});
