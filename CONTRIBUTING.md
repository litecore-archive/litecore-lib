Contributing to Bibecore
=======

We're working hard to make *bibecore* the most powerful JavaScript library for working with bibecoin. Our goal is to have *bibecore* be a library that can be used by anyone interested in bibecoin, and to level expertise differences with great design and documentation.
...
These are some global design goals in bibecore that any change must adhere.
...
When possible, bibecore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Bibecore methods. For example, calling `JSON.stringify` on a Bibecore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'bibecore' and Look up Classes from There
...
var PublicKey = bibecore.PublicKey;
```
DON'T:
```javascript
...
```
Finally go to [github.com/Bitbibe/bibecore-lib](https://github.com/Bitbibe/bibecore-lib) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of bibecore.

If you have any questions feel free to post them to
[github.com/Bitbibe/bibecore-lib/issues](https://github.com/Bitbibe/bibecore-lib/issues).
